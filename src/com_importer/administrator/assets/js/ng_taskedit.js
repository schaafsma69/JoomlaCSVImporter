...
taskeditApp.controller('taskeditCtrl', function($scope, $http) {
	...
	$scope.removecsv = function( csvid ) {
		for(var i=0;i<$scope.csvfields.length;i++) {
			if ($scope.csvfields[i].id == csvid) {
				$scope.csvfields.splice(i,1);
				return;
			}
		}
	}
	...
	$scope.addCSVField = function(name) {
		for(var i=0;i<$scope.csvfields.length;i++) {
			if ($scope.csvfields[i].id == name) return;
		}
		var csv_object = { id:	name, name:	name, format: "text", is_empty_allowed: true};
		$scope.csvfields.push( csv_object );
	}
	...
	$scope.getxml = function() {
		...
		$scope.xml_file = str_xml + "</tables></output></definition>";
	}
